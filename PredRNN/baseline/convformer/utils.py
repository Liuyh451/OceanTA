...
class covlstmformer(nn.Module):
    def __init__(self, configs):
        ...
        # 5：步幅（stride）
        self.cov1 = Cov(3, 8, 3, 3)
        self.cov2 = Cov(3, 8, 3, 3)
        # 两个编码器
        ...
        self.cov_last = Cov_last(3, 8, 3, 1)
...
def fold_tensor(tensor, output_size, kernel_size):
    ...
    Returns:
        torch.Tensor: 重构后的图像张量，形状为 (N, *, C, H, W)
            ...
            - W = n_w * k_w
    """
    # 重塑张量维度：(batch, num_patches=625,T=3, patch_features=25)
    # 625 = (H/k_h) * (W/k_w) = (128/5)*(128/5) 当kernel_size=5时
    tensor = tensor.reshape(-1, 625, 3, 25)  # (batch, num_patches=625, T=3, patch_features=25)

    # 获取时间步长维度数（此处T=3对应滑动窗口）
    T = tensor.size(2)

    # 维度置换：(batch, T, patch_features, num_patches)
    tensor = tensor.permute(0, 2, 3, 1)

    # 重塑为空间维度布局：(batch, T, patch, H/k_h, W/k_w)
    tensor = tensor.reshape(tensor.size(0), T, 25, 25, 25)
    tensor = tensor.float()
    ...
    # 处理不同维度情况：展平batch和通道维度
    f = tensor.flatten(0, 1) if n_dim == 5 else tensor

    # 核心折叠操作：将滑动窗口块重组为完整图像
    folded = F.fold(f.flatten(-2),
    ...
    # 最终形状调整：(batch, T, channel,128, 128)
    return folded.reshape(-1, T, 3, 128, 128)
...
class NoamOpt:
    """
    实现Noam学习率调度策略，用于Transformer模型的训练预热和衰减
    根据论文"Attention Is All You Need"中的学习率公式实现
    
    Args:
        model_size (int): 模型的特征维度(d_model)，用于缩放学习率
        factor (float): 缩放因子，控制学习率的整体幅度
        warmup (int): 预热步数，在此期间学习率线性增长
        optimizer (torch.optim.Optimizer): 要包装的优化器对象
    """
    ...
    def rate(self, step=None):
        ...
        Args:
            step (int, optional): 指定计算步数，默认使用内部计数器_step
