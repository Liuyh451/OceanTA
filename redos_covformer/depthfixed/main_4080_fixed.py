...
import numpy as np
dynamic_depth=1
full_path = "/home/hy4080/wplyh/depthnew/hotfix/depth1/temp"
...
""""
train_argo不变，永远为表层，test_argo对应原代码train_argo[:-12,:]，用来测试的，也是不变
label_argo随深度变化，label_test_argo对应原代码label_argo[-12:]
"""
...
X_test = sta_test
true_test = true_test
print("true_test.shape", true_test.shape, "X_train.shape", X_train.shape)
...
# 保存配置和训练结果
trainer.save_configs(f'{full_path}/config_train.pkl')
trainer.train(dataset_train, dataset_eval, f'{full_path}/checkpoint.chk')
...
chk = torch.load(full_path + '/checkpoint.chk')
...
np.save(full_path + "/data/test_pred.npy", test_pred)
np.save(full_path + "/data/test_true.npy", test_true)
...
# todo depth更改3
nrmse = loss(data_mask_t, dynamic_depth, test_pred, test_true)
