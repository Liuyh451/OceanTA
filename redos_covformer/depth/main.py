...
#划分训练集
train_sssa, _, _ = read_raw_data('t', 23, 3, data_dict)
...
train_argo, label_argo, data_mask_t_1 = read_raw_data('s', depth_globel, 3, data_dict)
#划分验证集
test_sssa, _, _ = read_raw_data('t', 23, 3, data_dict_Jan)
...
test_argo, label_test_argo, data_mask_t_2 = read_raw_data('s', depth_globel, 3, data_dict_Jan)
...
configs.dim_feedforward =1024
#表示在模型中使用的dropout率为0.3。Dropout是一种正则化技术，用于减少过拟合。
configs.dropout = 0.6
#表示编码器的层数为4。这意味着模型有4个堆叠的编码器层
configs.num_encoder_layers = 6
configs.num_decoder_layers = 6
