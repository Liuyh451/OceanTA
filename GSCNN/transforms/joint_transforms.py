    ...
    def __call__(self, img, mask,edge):
        ...
        for attempt in range(10):
            ...
            if w <= img.size[0] and h <= img.size[1]:
                ...
                edge = edge.crop((x1, y1, x1 + w, y1 + h))
                assert (img.size == (w, h))

                return img.resize((self.size, self.size), Image.BICUBIC),\
                    mask.resize((self.size, self.size), Image.NEAREST), \
                    edge.resize((self.size, self.size), Image.NEAREST)

        # Fallback
        scale = Scale(self.size)
        crop = CenterCrop(self.size)
        return crop(*scale(img, mask,edge))
...
class RandomSizeAndCrop(object):
    ...
    def __call__(self, img, mask,edge, centroid=None):
        assert img.size == mask.size
        ...
        img, mask, edge = img.resize((w, h), Image.BICUBIC), mask.resize((w, h), Image.NEAREST), edge.resize((w, h), Image.NEAREST)

        return self.crop(img, mask, edge, centroid)
