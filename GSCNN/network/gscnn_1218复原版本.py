...
class GSCNN(nn.Module):
    ...
    def forward(self, inp,input2,edge, gts=None):
        ...
        # res 4-7 [2,512,15,13] [2,1024,15,13] [2,2048,15,13] [2,4096,15,13]
        m4 = self.mod4(m3)
        m5 = self.mod5(m4)
        m6 = self.mod6(m5)
        m7 = self.mod7(m6) 
        ...
        cat = torch.cat((edge_out, canny_edge), dim=1)#[ 2,2,120,101]       ·         -------------------第一层边缘信息
        acts = self.cw(cat)# [ 2,1,120,101]
        acts = self.sigmoid(acts) # [ 2,1,120,101]
        # output = self.aspp(feature_map) # (shape: (batch_size, num_classes, h/16, w/16))
        # aspp
        x = self.aspp(m7, acts)# [2,4096,15,13] [ 2,1,120,101]----》[ 2,1536,,15,13]  --------------------第二层边缘信息
