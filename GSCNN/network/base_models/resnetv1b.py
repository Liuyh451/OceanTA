...
def resnet50_v1s(pretrained: object = False, root: object = '~/.torch/models', **kwargs: object) -> object:
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], deep_stem=False, **kwargs)
    if pretrained:
        # model = nn.DataParallel(model).cuda()


        # import model_store.get_resnet_file
        # from ..model_store import get_resnet_file
        # 获取当前python文件所在目录:
        path_1 = os.path.split(os.path.realpath(__file__))[0]
        print("-----------1:"+path_1)
        print("-----------2:"+os.getcwd())

        root_path = os.path.abspath(os.path.dirname(os.getcwd()))
        # path_model_file = os.path.join(root_path,"resnet50-19c8e357.pth")
        path_model_file = os.path.join(root_path,"resnet50-0676ba61.pth")
        # model.load_state_dict(torch.load(path_model_file)['state_dict'], False)
        # 获取工作目录：

        # path_model_file="../../resnet50-0676ba61.pth"

        model.load_state_dict(torch.load(path_model_file), strict=False)
