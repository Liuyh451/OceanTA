...
from network.base_models.xception import get_xception
from network.dp_eddy import _ASPP
from network.fcn import _FCNHead
...
def get_deeplabv3_plus(dataset='pascal_voc', backbone='xception', pretrained=False, root='~/.torch/models',
    ...
    model = DeepLabV3Plus(3, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    pretrained = True
    # temp1 = backbone
    # temp = acronyms[dataset]
    if pretrained:
        root_path = os.path.abspath(os.path.dirname(os.getcwd()))
        root_path = os.getcwd()
        # path_model_file = os.path.join(root_path,"resnet50-19c8e357.pth")
        path_xception = os.path.join(root_path,"deeplabv3plus_xception_VOC2012_epoch46_all.pth")
        ...
        #         map_location=device))
        model.load_state_dict(
            torch.load(path_xception), strict=False)
    return model
