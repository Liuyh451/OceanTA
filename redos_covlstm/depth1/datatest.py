...
file_path = './REDOS_1.0_19920101.nc'
file_obj = nc.Dataset(file_path)

lon_u = file_obj.variables['lon_u']
lon_v = file_obj.variables['lon_v']
lat_u = file_obj.variables['lat_u']
lat_v = file_obj.variables['lat_v']
zeta = file_obj.variables['zeta']
depth = file_obj.variables['depth']


# for file in nc_files:  # 假设 nc_files 是你的 NetCDF 文件路径列表
#     with nc.Dataset(file, 'r') as nc_file:
#         # 提取 lv=0 的数据
#         temp_lv0 = nc_file.variables['t'][0, :, :]  # 假设 lv=0 对应第一个索引
...
sta_train = data_reduced(sta_train, 0)
ssa_train = data_reduced(ssa_train, 0)
ssha_train = data_reduced(ssha_train, 0)
sswu_train = data_reduced(sswu_train, 1)
sswv_train = data_reduced(sswv_train, 2)
sta_train.shape, ssa_train.shape, ssha_train.shape, sswu_train.shape, sswv_train.shape

sta_test = data_reduced(sta_test, 0)
ssa_test = data_reduced(ssa_test, 0)
ssha_test = data_reduced(ssha_test, 0)
sswu_test = data_reduced(sswu_test, 1)
sswv_test = data_reduced(sswv_test, 2)
label_argo = label_argo[:, :, :-1, :-1]
sta_test.shape, ssa_test.shape, ssha_test.shape, sswu_test.shape, sswv_test.shape, label_argo.shape
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        ntime = query.size(2)
        # (N, h, S, T, d_k)
        query, key, value = [l(x).view(x.size(0), x.size(1), x.size(2), self.nheads, self.d_k).permute(0, 3, 1, 2, 4)
...
class Trainer:
    def __init__(self, configs):
        ...
        #计算学习率调度，configs.d_model: 模型的维度，configs.warmup学习率预热的超参数
        #乘积的平方根。这个操作是为了在学习率调度中调整因子的大小，以平衡训练过程中的学习率变化。
